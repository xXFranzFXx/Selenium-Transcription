...
public class YoutubePageTests extends BaseTest {
    ...
    public void writeClosedCaptionsToFile(String youtubeURL) throws ExecutionException, InterruptedException {
        ...
        CompletableFuture<Object> reqID = CompletableFuture.supplyAsync(() -> {
        ...
        CompletableFuture<Void> resBody = reqID.thenApply(id ->
                devTools.addListener(Network.responseReceived(), responseReceived -> {
                    String body = null;
                    body = devTools.send(Network.getResponseBody(rI)).getBody();
                    try {
                        writeFile(body);
                    } catch (IOException e) {
